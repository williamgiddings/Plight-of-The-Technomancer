...
float2 VAT_uvPosition(float frameStartOffset, float2 uvIndex, int numOfFrames, float speed, float time, float2 paddedRatio)
{
    float2 uvPosition;
    float FPS = 24.0;
    ...
    timeInFrames = clamp(timeInFrames + (frameStartOffset / numOfFrames), 0, 1.0-(frameStartOffset / numOfFrames));
...
{
    float2 uvPosition = VAT_uvPosition(0.0, uvIndex, numOfFrames, speed, time, paddedRatio);
...
void VAT_Soft_float(
    ...
    float2 paddedRatio,
    bool packNorm,
    float FrameStartOffset,
    out float3 outPosition,
    out float3 outNormal
)
{
    float2 uvPosition = VAT_uvPosition(FrameStartOffset, uvIndex, numOfFrames, speed, time, paddedRatio);
